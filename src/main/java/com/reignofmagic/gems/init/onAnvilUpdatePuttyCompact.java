...
public final class onAnvilUpdatePuttyCompact {
	...
	private static final int RENAME_COST = 0;
	private static final int[] EXPERIENCE_COST = { 30 };
	private static final int[] REPAIR_AMOUNT_ = { 32700 };


	private boolean isValidRepairItem(final ItemStack stack) {

		if (stack != null) {
			return stack.getItem() == ModItems.gemPuttyCompact;
		}
	...
	public void onAnvilChange(final AnvilUpdateEvent event) {
		...
		if (canBeRepaired(itemToRepair)) {
			...
			if (repairMaterial.getItem() == ModItems.gemPuttyCompact)
				repairAmount = 32700;
			else
				repairAmount = 32700;
	...
	public onAnvilUpdatePuttyCompact() {
	}
